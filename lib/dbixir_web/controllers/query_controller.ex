defmodule DbixirWeb.QueryController do
  ...
  def show_query_area(conn, _params) do
    render conn, "query_area.html", query_string: String.strip(""), rows: [], columns: []
  end
  ...
  def query_execute(%Plug.Conn{body_params: body_params, cookies: cookies} = conn, _params) do
    ...
    if pid do
      ...
      if query_string do
        ...
        render conn, "query_area.html", rows: rows, columns: columns, query_string: String.trim(query_string)
      else
        conn = conn |> put_flash(:error, "Query string is empty.")
        render conn, "query_area.html", rows: [], columns: [], query_string: String.trim("")
      end
    else
